...
const schema = z.object({
  ...
  valor_potencial:     z.number({ invalid_type_error: "Debe ser un número" }).positive("Mayor a 0"),
  servicio_adquirido:  z.string().optional().default(""),
  setter_asignado:     z.string().optional().default(""),
  usuario_de_origen:   z.string().optional().default(""),
  fecha:               z.string().min(1, "Requerido"),
  fecha_de_la_reunion: z.string().optional().default(""),
  notas:               z.string().max(800).optional().default(""),
  cerro_la_venta:      z.boolean().optional().default(false),
});
...
export default function LeadModal({ isOpen, lead, isSubmitting, onClose, onSubmit }: Props) {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<FormValues>({
    ...
    } : { fecha: new Date().toISOString().split("T")[0], cerro_la_venta: false },
  });

  const submit = async (v: FormValues) => {
    await onSubmit({ ...v, valor_potencial: v.valor_potencial, cerro_la_venta: v.cerro_la_venta ?? false });
    if (!isEdit) reset();
  };
