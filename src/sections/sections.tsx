...
export function TeamSection() {
  ...
  const setterMap: Record<string, {
    total: number; cerrados: number; activos: number;
    revenue: number; pipeline: number; servicios: Record<string, number>;
  }> = {};
                ...
                {ranking.map(([name, s], i) => {
                  ...
                  return (
                    <tr key={name} className="hover:bg-slate-800/25 transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <span className="text-sm font-bold text-white tabular-nums">{formatCurrency(s.revenue)}</span>
                      </td>
                    </tr>
                  );
                })}
              ...
              return (
                <div key={name} className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    {[["Total", s.total], ["Cerrados", s.cerrados], ["Activos", s.activos]].map(([l, v]) => (
                      <div key={l as string} className="bg-slate-800/50 rounded-lg p-2.5 text-center">
            ...
            {items.map(tool => (
              <a key={tool.name} href={tool.link} target="_blank" rel="noopener noreferrer"
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="flex-shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-400">
                    {tool.tag}
                  </span>
                </div>
    ...
    <div className="p-5 md:p-6 max-w-2xl mx-auto space-y-5">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <p className="text-xs text-slate-500 leading-relaxed mb-3">
          Los datos se sincronizan en tiempo real. Cualquier cambio de un setter aparece inmediatamente en tu vista de administrador.
        </p>
